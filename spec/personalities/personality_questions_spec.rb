...
describe PersonalityScoring do
  describe '#reverse_count' do
    ...
    it 'returns 1 when score is 5' do
      expect(Foo.new(personality_questions).reverse_count(personality_questions[:one_speaker])).to eql 1
    end

    it 'returns 3 when score is 3' do
      expect(Foo.new(personality_questions).reverse_count(personality_questions[:six_quiet])).to eql 3
    end
  end
